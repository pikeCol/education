...
import { getShareDetail, paperPrint, paperLog } from '@/services/questions/detail';
...
const QuestionDeatailContent = (props) => {
  ...
  const ct = data.contents.map((v, index) => {
    ...
    return (
      <div key={v.id} className="question-part">
        <div className="question">
          <div className="desc" dangerouslySetInnerHTML={{ __html: `${question}` || '' }} />
          <div className="select">
            {options?.map((x) => (
              <span key={x} dangerouslySetInnerHTML={{ __html: x || '' }} />
            ))}
          </div>
        </div>
...
const ShareDetail = (props) => {
  ...
  const downloadPage = () => {
    ...
    paperLog({ id: params.id, operateType: 'download' });
  ...
  return (
    <PageHeaderWrapper>
      <div className="detail">
        ...
        <div id="content">
          {detail.content &&
            detail.content.map((v, index) => {
              return (
                <QuestionDeatailContent
                  currentUser={currentUser}
                  refresh={() => useDetail(params)}
                  key={index}
                  className="content"
                  paperId={params.id}
                  data={v}
